...
(function(){
  var WINDOW, philtre, map, loadEntriesThen, makeHitDiv, makeEmbed, search, pointerHandler, input, slice$ = [].slice;
  WINDOW = 5;
  ...
  makeEmbed = function(entry){
    ...
    url = entry.image;
    if (entry.image && !entry.image.match('https?://')) {
      parts = entry.image.split('/');
      url = slice$.call(parts, 0, -2 + 1 || 9e9).join('/') + '/thumbs/' + parts[parts.length - 1];
    }
