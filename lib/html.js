...
(function(){
  ...
  addMetaTags = function(dom, entry){
    ...
    if (entry.firstImage && 'http' !== entry.firstImage.substr(0, 4)) {
      ...
      entry.firstImage = readConfig().url + entry.firstImage;
    }
  ...
  buildSiteHtml = function(root, entries){
    ...
    for (i$ = 0, len$ = entries.length; i$ < len$; ++i$) {
      ...
      if (entry.slug) {
        fs.symlinkSync(root + "" + suffix + ".html", root + "/" + entry.slug + ".html");
      }
