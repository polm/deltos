...
(function(){
  var fs, ref$, markdown, getMtime, memoize, isIn, tagged, yaml, yamlDump, deltosHome, readConfig, getFilename, getUrl, getAllEntries, getRawEntry, map, take, sortBy, sortWith, reverse, dirname, renderBlock, domino, RSS, eep, Section, render, buildSite, buildSiteCore, cleanDir, flagUpdated, dumpJson, entriesToJson, htmlInit, buildPageCore, buildPageHtml, buildPageData, addMetaTags, defaultImage, setMeta, getMetaData, getRenderedEntries, beginsWith, RENDERED_CACHE, readEntryBody, getTemplate, searchableText, entryRules, buildSiteHtml, buildRss, makeRssFile, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice;
  fs = require('fs-extra');
  ref$ = require('./util'), markdown = ref$.markdown, getMtime = ref$.getMtime, memoize = ref$.memoize, isIn = ref$.isIn, tagged = ref$.tagged, yaml = ref$.yaml, yamlDump = ref$.yamlDump, deltosHome = ref$.deltosHome, readConfig = ref$.readConfig, getFilename = ref$.getFilename, getUrl = ref$.getUrl;
  ...
  addMetaTags = function(dom, entry){
    ...
    dom.querySelector("meta[name=\"description\"]").setAttribute('content', metadata.description);
  ...
  buildRss = function(root, config, entries){
    var philtre, name, ref$, query, entriesCurrent, results$ = [];
    makeRssFile(config, root, 'index', entries);
    config.rss = config.rss || {};
    philtre = require('philtre').philtre;
    for (name in ref$ = config.rss) {
      query = ref$[name];
      entriesCurrent = philtre(query, entries);
      results$.push(makeRssFile(config, root, name, entriesCurrent));
    }
    return results$;
  };
  makeRssFile = function(config, root, name, entries){
    var rss, i$, ref$, len$, entry, body;
    rss = new RSS({
      title: config.title,
    ...
    for (i$ = 0, len$ = (ref$ = entries.slice(0, 5)).length; i$ < len$; ++i$) {
      entry = ref$[i$];
      body = entry.body.replace(/href="\//g, "href=\"" + config.url + "/").replace(/src="\//g, "src=\"" + config.url + "/");
      rss.item({
        title: entry.title,
        date: entry.date,
        description: body,
    ...
    return fs.writeFileSync(root + name + ".rss", rss.xml());
  };
}).call(this);
