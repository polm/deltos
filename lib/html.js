...
(function(){
  ...
  out$.buildPrivateReference = buildPrivateReference = function(){
    return buildSiteCore(getAllEntries(), deltosHome + 'private/');
  };
  out$.buildSite = buildSite = function(){
    var entries, rssEntries, root, after;
    entries = getAllEntries().filter(tagged('published'));
    rssEntries = entries.filter(function(it){
    ...
    return buildSiteCore(entries, root, rssEntries, after);
  };
  buildSiteCore = function(entries, siteRoot, rssEntries, after){
    htmlInit();
    ...
    if (rssEntries) {
      ...
      rssEntries));
      buildRss(siteRoot, readConfig(), rssEntries);
    }
