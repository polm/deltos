...
(function(){
  var fs, ref$, getMtime, memoize, isIn, tagged, yaml, yamlDump, deltosHome, readConfig, getFilename, getSlug, getAllEntries, getRawEntry, map, take, sortBy, sortWith, reverse, renderBlock, Markdown, markdown, domino, RSS, eep, Section, render, buildPrivateReference, buildSite, buildSiteCore, cleanDir, flagUpdated, dumpJson, entriesToJson, htmlInit, buildPageCore, buildPageHtml, buildPageData, addMetaTags, defaultImage, setMeta, getMetaData, getRenderedEntries, beginsWith, RENDERED_CACHE, readEntryBody, getTemplate, searchableText, entryRules, buildSiteHtml, buildRss, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice;
  fs = require('fs-extra');
  ref$ = require('./util'), getMtime = ref$.getMtime, memoize = ref$.memoize, isIn = ref$.isIn, tagged = ref$.tagged, yaml = ref$.yaml, yamlDump = ref$.yamlDump, deltosHome = ref$.deltosHome, readConfig = ref$.readConfig, getFilename = ref$.getFilename, getSlug = ref$.getSlug;
  ...
  flagUpdated = function(root, entries){
    ...
    for (i$ = 0, len$ = entries.length; i$ < len$; ++i$) {
      ...
      if (getMtime(htmlFname) < getMtime(getFilename(entry.id) + '/meta')) {
        entry.updated = true;
      }
