...
(function(){
  ...
  ref$ = require('./blocks'), renderBlock = ref$.renderBlock, getSlug = ref$.getSlug;
  ...
  cleanDir = function(root, entries){
    var files, i$, len$, file, id, results$ = [];
  ...
  readEntryBody = function(entry){
    ...
    for (i$ = 0, len$ = (ref$ = entry.rawBody.split("\n\n")).length; i$ < len$; ++i$) {
      block = ref$[i$];
      expanded += renderBlock(block, entry);
    }
