...
(function(){
  ...
  out$.allEntriesCacheFirst = allEntriesCacheFirst = function(entries, transformer, progress){
    ...
    findFresh = function(cache){
      ...
      edict = {};
      for (i$ = 0, len$ = (ref$ = cache.entries).length; i$ < len$; ++i$) {
        entry = ref$[i$];
  ...
  out$.getAllEntriesAsync = getAllEntriesAsync = function(entries, transformer, progress, finish){
    var files, cache, fobs, readFile;
    files = fs.readdirSync(BASEDIR);
    cache = loadFullCache();
    fobs = files.map(function(it){
      return {
        fname: it,
        date: getMtime(getFilename(it) + '/deltos')
      };
    });
    fobs.sort(revDate);
    files = fobs.map(function(it){
      return it.fname;
    });
    readFile = function(){
      var id, entry;
      ...
      id = files.shift();
      entry = cache.entries[id] || readEntry(id);
      if (transformer) {
        entry = transformer(entry);
      }
      entries.push(entry);
  ...
  out$.getAllEntries = getAllEntries = memoize(function(){
    var cache, entryList, entries, i$, len$, entry, cdate, ref$, ff, base, key, parent;
    cache = loadFullCache();
