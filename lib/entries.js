...
(function(){
  var ref$, memoize, localIsoTime, getYesterday, yaml, yamlDump, deltosHome, BASEDIR, getFilename, tagged, getSlug, getMtime, fs, uuid, map, filter, values, sortBy, reverse, newNote, dumpTodos, renderTsvEntry, dumpTsv, dumpTsvTagged, dumpTsvCore, grepEntries, philtreEntries, readEntry, loadFullCache, writeFullCache, loadMetaCache, writeMetaCache, loadCache, allEntriesCacheFirst, loadCacheAsync, revDate, writeCache, getAllEntries, getRawEntry, getEntryParts, getNewId, newDaily, out$ = typeof exports != 'undefined' && exports || this;
  ...
  readEntry = function(id){
    ...
    if (metadata.parents) {
      if (metadata.parents.length === 1) {
        ...
        metadata.collections = "This post is part of a collection on <a href=\"/by-id/" + collection.id + "/#" + getSlug(collection) + "\">" + collection.title + "</a>.";
      } else {
        ...
        for (i$ = 0, len$ = (ref$ = metadata.parents).length; i$ < len$; ++i$) {
          ...
          colls.push("<a href=\"/by-id/" + collection.id + "/#" + getSlug(collection) + "\">" + collection.title + "</a>");
        }
