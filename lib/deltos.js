...
(function(){
  var ref$, readStdinAsLinesThen, launchEditor, deltosHome, getFilename, fs, init, newNote, newDaily, dumpTsv, dumpTodos, writeDaily, writePost, editPost, render, buildSite, buildPrivateReference, allToJson, commands, addCommand, func, e;
  ...
  addCommand('json', "Dump all entries to JSON", function(){
    return console.log(allToJson());
  });
  addCommand('cache', "Cache json dump", function(){
    fs.writeFileSync(deltosHome + 'deltos.cache.json', allToJson());
    return fs.writeFileSync(deltosHome + 'deltos.cache.tsv', dumpTsv());
  });
