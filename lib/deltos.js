...
(function(){
  var ref$, launchEditor, deltosHome, getFilename, readConfig, editConfig, installTheme, newNote, newDaily, dumpTsv, dumpTsvTagged, dumpTodos, grepEntries, philtreEntries, commands, addCommand, func, e, slice$ = [].slice;
  ...
  addCommand("post [title...]", "Start a new post in $EDITOR", function(){
    var args;
    args = slice$.call(arguments);
    return launchEditor(newNote(args.join(' ')));
  });
  addCommand("edit [id]", "Edit an existing post", function(it){
    return launchEditor(getFilename(it));
  });
