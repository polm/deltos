...
(function(){
  var fs, Yaml, yaml, markdown, ref$, Obj, filter, keys, values, groupBy, concat, unique, map, take, sortBy, sortWith, reverse, intersection, uuid, ls, domino, RSS, eep, Section, memoize, deltosHome, BASEDIR, getFilename, evalLs, isIn, tagged, init, readConfig, writePost, writeDaily, editPost, normalizeDate, readEntry, readEntryFromFile, getChildEntries, getAllEntries, localIsoTime, getNewId, newNote, newDaily, noEmpty, readStdinAsLinesThen, launchEditor, dumpTodos, dumpTsv, htmlInit, beginsWith, readEntryBody, getTemplate, searchableText, buildPage, entryRules, deltosLinkToHtml, getRenderedEntries, render, toMarkdownLink, buildListPage, sortOrderThenDate, buildHierarchicalList, buildPrivateReference, buildSite, allToJson, commands, addCommand, func, e;
  fs = require('fs');
  ...
  buildSite = function(priv){
    var config, published, siteRoot, entries, i$, len$, entry, page, fname, rss, ref$, tag;
    ...
    return fs.writeFileSync(siteRoot + "index.rss", rss.xml());
    function fn$(it){
      return !tagged(tag, it);
    }
  };
